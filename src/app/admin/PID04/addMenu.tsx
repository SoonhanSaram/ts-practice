...
const AddMenu = ({modal} : AddMenuProps,) => {
    ...
    const addMenu = async () => {
        ...
        inputValue['upper_url'] === null ? setInputValue((prevValues: any) => ({...prevValues, 'upper_url' : ''})) : setInputValue(inputValue);
        ...
        <div className="modal">
            ...
            <div>
                {field.map((input) => { 
                    return (
                        <div>
                            <label htmlFor={input.id}>{input.title}</label> <input id={input.id} onChange={(e) => onChangeValue(e)} value={inputValue[input.id]}/> {input.title == "메뉴 권한" && (<p>'admin', 'user' 만 사용가능</p>)}<br/>
                        </div>
                    )
                })}
            </div>
            <div>
                ...
                <button onClick={modal}>취 소</button>
            </div>
        </div>
    )
}
